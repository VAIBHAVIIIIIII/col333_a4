...
inline Factor product(const Factor &a, const Factor &b)
{
    Factor dummy;
    return dummy; // TODO: insert actual product function
}

inline Factor sum_out(const Factor &f, int var)
{
    Factor dummy;
    return dummy; // TODO: insert actual sum_out function
}

inline Factor restrict_factor(const Factor &f, int var, int val)
{
    Factor dummy;
    return dummy; // TODO: insert actual restrict function
}
...
network read_network(const char *filename)
{
    ...
    while (getline(f, line))
    {
        ...
        else if (token == "probability")
        {
            ...
            if (op == string::npos || cp == string::npos)
                continue;
                    ...
                    while (getline(tss, num, ','))
                    {
                        ...
                        if (num == "?" || num == "-1" || num == "-1.0")
                            ...
                            try {
                                ...
                                // Debug print
                                std::cerr << "Warning: Non-numeric CPT entry: '" << num << "', replacing with -1\n";
                                v = -1.0f;
                            }
                        }
                        cpt.push_back(v);
            ...
            for (int k = 0; k < nval; ++k)
            {
                ...
                for (int n = 0; n < N; ++n)
                {
                    ...
                    for (const string &p : pars)
                        if (tmp.find(net.get_index(p)) == tmp.end())
                        {
                            ok = false;
                            break;
                        }
